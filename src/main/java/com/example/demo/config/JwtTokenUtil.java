...
//        return doGenerateToken(claims, userPrincipal.getUsername());
    ...
    public JwtUserInfoDTO getJwtUserInfo(String token) throws JsonProcessingException {
        Claims claims = Jwts.parser().setSigningKey(secret).parseClaimsJws(token).getBody();
        String staffJson = (String) claims.get("staff");
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.setSerializationInclusion(JsonInclude.Include.NON_NULL);
        JwtUserInfoDTO jwtUserInfoDTO = objectMapper.readValue(staffJson,JwtUserInfoDTO.class);
